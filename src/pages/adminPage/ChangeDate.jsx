...
import { logo } from '../../assets';  // ตรวจสอบให้แน่ใจว่า `logo` คือพาธรูปภาพสำรอง

function ChangeDate() {
  const [reservations, setReservations] = useState([]);
  const [timeSelections, setTimeSelections] = useState({});  // เก็บเวลาที่เลือกสำหรับแต่ละการจอง

  // ดึงข้อมูลการจองจาก Backend
  useEffect(() => {
    fetch('http://localhost:5000/api/change-date')  // ใส่ URL ของ Backend ถ้าอยู่บนพอร์ตอื่น
      .then((response) => {
        if (!response.ok) {
          throw new Error('ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้'); // ตรวจสอบการเชื่อมต่อ
        }
          ...
          <div className="col-xl-10 col-lg-2 col-md-2 pt-3">
            <table className="table table-striped table-hover">
              <thead>
                <tr>
                  <th style={{ width: '3%' }}>&nbsp;</th>
                  <th style={{ width: '4%' }}>ผู้จอง</th>
                  <th style={{ width: '3%' }}>รหัส</th>
                  <th style={{ width: '3%' }}>วันจอง</th>
                  <th style={{ width: '3%' }}>วันที่รับ</th>
                  <th style={{ width: '4%' }}>เลื่อนวันที่เป็น</th>
                  <th style={{ width: '4%' }}>เลื่อนเวลาเป็น</th>
                  <th style={{ width: '4%' }}>&nbsp;</th>
                </tr>
              </thead>
              <tbody>
                {reservations.map((reservation) => {
                  ...
                  return (
                    <tr key={reservation.booking_id}>
                      ...
                      <td>{new Date(reservation.pickup_date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
                      <td>
